...
from stage_1_special_replacements import (
    replacements1, replacements2, replacements3,
    allowed_values_vertical, allowed_values_horizontal_agro,
...
class HeaderProcessor:
    ...
    def apply_special_replacements(self, header_rows: List[List[str]]) -> List[List[str]]:
        ...
        for i, row in enumerate(header_rows):
            row = [str(cell).strip() if pd.notna(cell) else cell for cell in row]
