...
def get_excluded_insurers(
    ...
    if not isinstance(selected_insurers, list) or not isinstance(top_n_groups, dict):
        logger.debug("Invalid input types for get_excluded_insurers")
        return set(), False
...
def setup_insurer_selection(app: Dash) -> None:
    ...
    def update_insurers_selections(
        ...
        try:
            ...
            selected_insurers = [v for v in selected_insurers if v is not None]
            ...
            logger.debug(f"options: {insurer_options_store['insurer_options']}")
