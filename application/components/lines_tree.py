...
from domain.lines.tree import Tree
...
class TreeRenderer:
    ...
    def create_tree_item(
        ...
        if has_selected_descendants:
            checkbox_classes.append(StyleConstants.TREE[
                                    "PARENT_OF_SELECTED"])
        if is_selected:
            checkbox_classes.append(StyleConstants.TREE[
                                    "SELECTED"])
        ...
        def create_subtree(code: str, level: int = 0
            ...
            if children and is_expanded:
                ...
                if child_items:
                    items.append(
                        dbc.Collapse(
                            html.Div(child_items, className=StyleConstants.TREE[
                                     "TREE_CHILDREN"]),
        ...
        if selected:
            for code in selected:
                if tree.node_exists(code):
                    # additional text cleanup if needed.
