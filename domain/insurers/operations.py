...
def get_top_insurers(df: pd.DataFrame, top_n: int = 0) -> List[str]:
    ...
    ) if top_n > 0 else value_sums.sort_values(ascending=False).index.tolist()
...
def process_insurers_df(df: pd.DataFrame, latest_df: pd.DataFrame,
               top_insurers: int, split_mode: str) -> pd.DataFrame:
...
def get_rankings(df: pd.DataFrame, quarters: List[str]
    ...
    def calculate_quarter_rankings(quarter_df: pd.DataFrame
        ...
        return {
            line: dict(zip(
                line_df.sort_values('value', ascending=False)
                ['insurer'].astype(str),
    ...
    prev_df = df[df[
     'year_quarter'] == quarters[-2]] if len(quarters) > 1 else pd.DataFrame()
