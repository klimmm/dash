...
class TableTransformer:
    ...
    @staticmethod
    def _get_rank_change(current: Optional[int], previous: Optional[int]) -> str:
        """Calculate rank change with formatting."""
        # Handle cases where either value is None
        if current is None:
            logger.error(f"current is none - previous {previous} ")
    ...
    def _get_ordered_metrics(self, metrics: np.ndarray) -> pd.DataFrame:
        ...
        return metric_groups.sort_values(['root_order', 'metric'])
    ...
    def _process_group(self, df: pd.DataFrame,
        ...
        df['year_quarter'] = pd.to_datetime(
            df['year_quarter']).dt.to_period('Q').astype(str)
        ...
        summary_mask = pivot_df[
            self.INSURER_COL].str.lower().str.contains(r'^top|^total', na=False)
        regular_rows = pivot_df[~summary_mask].copy()
        summary_rows = pivot_df[
            summary_mask].copy() if summary_mask.any() else None
        ...
        if current_ranks:
            regular_rows[self.PLACE_COL] = regular_rows.apply(
                lambda row: self._get_rank_change(
                    ...
                    prev_ranks.get(row[self.INSURER_COL]) if prev_ranks
                    else None
                ) if split_mode == 'line' else self._get_rank_change(
                    current_ranks.get(str(row[self.LINE_COL]).lower(), {}).get(
                        row[self.INSURER_COL]),
                    prev_ranks.get(str(row[self.LINE_COL]).lower(), {}).get(
                        row[self.INSURER_COL]) if prev_ranks else None
                ),
                axis=1
            )
        ...
        if summary_rows is not None and not summary_rows.empty:
            ...
            result_df = pd.concat([regular_rows, summary_rows],
                                  ignore_index=True)
        else:
        ...
        result_df[self.INSURER_COL] = result_df[
            self.INSURER_COL].apply(map_insurer)
    ...
    def _sort_by_hierarchy(self, df: pd.DataFrame,
                           hierarchy: Dict) -> pd.DataFrame:
        """Sort DataFrame based on insurance line hierarchy."""
        def get_node_info(key, depth=0):
            if key in hierarchy:
    ...
    def transform_table(
        ...
        group_col, item_col = (
            self.LINE_COL, self.INSURER_COL) if split_mode == 'line' else (
        ...
        for group in df[group_col].unique():
            ...
            group_prev_ranks = prev_ranks.get(
                group_lower, {}) if (
                split_mode == 'line' and prev_ranks) else prev_ranks
            group_current_ranks = current_ranks.get(
                group_lower, {}) if (
                split_mode == 'line' and current_ranks) else current_ranks
        ...
        if split_mode == 'line':
            ...
            exclude_cols = {
                self.PLACE_COL, self.INSURER_COL,
                self.LINE_COL}
            metric_cols = [
            ...
            if metric_cols:
                ...
                summary_mask = result[
                    self.INSURER_COL].str.lower().str.contains(
                        r'^топ|^total|весь рынок', na=False)
                ...
                if not regular_rows.empty:
                    ...
                    regular_rows['_sort_value'] = pd.to_numeric(
                        regular_rows[sort_col].replace('-', float('-inf')),
                        errors='coerce'
                    )
                    regular_rows.sort_values('_sort_value',
                                             ascending=False, inplace=True)
                    regular_rows.drop(columns=['_sort_value'], inplace=True)
                    # Recombine sorted regular rows with summary rows
                    result = pd.concat([regular_rows, summary_rows],
                                       ignore_index=True)
