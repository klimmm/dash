...
def load_and_preprocess_data(file_path: str) -> pd.DataFrame:
    ...
    Load and preprocess insurance data from CSV file.
    
    Args:
        file_path (str): Path to the CSV file
        
    Returns:
        pd.DataFrame: Preprocessed DataFrame with proper date formatting
    ...
    try:
        ...
        # Ensure year_quarter is datetime
        ...
        return df.sort_values('year_quarter', ascending=True)
...
def create_year_quarter_options(df: pd.DataFrame) -> List[Dict[str, str]]:
    """
    @API_STABILITY: BACKWARDS_COMPATIBLE
    Create year-quarter options from DataFrame.
    
    Args:
        df (pd.DataFrame): DataFrame with 'year_quarter' column in datetime format
        
    Returns:
        List[Dict[str, str]]: List of year-quarter options in format {'label': 'YYYYQ1', 'value': 'YYYYQ1'}
    """
    # Convert to PeriodIndex to get proper quarter formatting
    unique_periods = pd.PeriodIndex(df['year_quarter'].dt.to_period('Q')).unique()
    
    return [
        {'label': period.strftime('%YQ%q'), 'value': period.strftime('%YQ%q')}
        for period in unique_periods
    ]


def get_insurer_options(df: pd.DataFrame, reporting_form: str) -> List[Dict[str, str]]:
    """
    @API_STABILITY: BACKWARDS_COMPATIBLE
    Generate insurer options based on reporting form and premium data.
    """
    # Filter metrics based on reporting form
    metrics = ['direct_premiums', 'inward_premiums'] if reporting_form == '0420162' else ['total_premiums']
    ...
    # Create pivot table for premiums
    ...
    if reporting_form == '0420162':
        pivot_df['total_premiums'] = pivot_df['direct_premiums'] + pivot_df['inward_premiums']
    
    # Sort insurers by total premiums
    sorted_insurers = pivot_df.sort_values('total_premiums', ascending=False).index
    
    return [{'label': map_insurer(insurer), 'value': insurer} for insurer in sorted_insurers]
